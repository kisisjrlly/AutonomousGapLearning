...
## 2026-09-22（CTBR 演示审计与可视化补接）

- 修正此前“安全闭环完成”的过度表述：仅执行了开环指令，未验证停止或恢复到重试状态；4/4 无接触不是协议完成。
- safe_probe_retreat 新增固定种子、逐帧 NPZ、阶段末速度/位移和显式 protocol_completed=false。
- 复用已有 Viewer 和 GIF，显示真实姿态与 COMMAND 阶段，记录动作前状态避免终止后 reset 污染。
- seed 0 实测撤退末最大速度 1.143 m/s，说明下一步必须实现反馈制动、恢复门槛，而不是直接宣称具备重试能力。
- 43 项测试通过。Rerun 可选依赖未安装，实际生成并交付 GIF，不声称交互 Viewer 已运行。
